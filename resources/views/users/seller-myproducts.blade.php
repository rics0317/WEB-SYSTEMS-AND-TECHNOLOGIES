...
@section('content')
    <div class="tabs">
        ...
        <a href="#" class="tab">Suspended</a>
        <a href="#" class="tab">Unlisted</a>
    </div>
    ...
    <div class="products-header">
        ...
        <div class="products-actions">
            <a href="{{ route('seller.add-product-step1') }}" class="add-product-button no-underline">
                    ...
                    <tr data-category="{{ $product->category }}" data-stock="{{ $product->stock }}" data-sales="{{ $product->sales }}">
                        ...
                        <td>
                            @foreach($product->variations as $variation)
                                <ul>
                                    @foreach(json_decode($variation->options, true) as $option)
                                        <li>{{ $option }}</li>
                                    @endforeach
                                </ul>
                            @endforeach
                        ...
                        <td>{{ $product->sales }}</td>
                        <td>
                            <a href="{{ route('seller.edit-product', $product->id) }}" class="edit-button btn-orange">Edit</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchButton.addEventListener('click', function() {
            ...
            window.location.href = `{{ route('seller.myproducts') }}?${queryString}`;
        });
        ...
        resetButton.addEventListener('click', function() {
            ...
            window.location.href = `{{ route('seller.myproducts') }}`;
        });
    });
</script>
